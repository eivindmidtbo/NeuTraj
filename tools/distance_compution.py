...
def trajectory_distance_batch(i, batch_trjs, trjs, metric_type = "hausdorff", data_name = 'porto'):
    ...
    pickle.dump(trs_matrix, open('./features/'+data_name+'_'+metric_type+'_distance_' + str(i), 'w'))
...
def trajectory_distance_combain(trajs_len, batch_size = 100, metric_type = "hausdorff", data_name = 'porto'):
    ...
    for i in range(1,trajs_len+1):
        if (i!=0) & (i%batch_size == 0):
            distance_list.append(pickle.load(open('./features/'+data_name+'_'+metric_type+'_distance_' + str(i))))
    ...
    print(all_dis.shape)
    pickle.dump(all_dis,open('./features/'+data_name+'_'+metric_type+'_distance_all_'+str(trajs_len),'w'))
    return all_dis
